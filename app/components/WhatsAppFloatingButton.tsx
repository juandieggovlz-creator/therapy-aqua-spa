...
export default function WhatsAppFloatingButton() {
	...
	if (isAdminPanel || !contenido?.whatsapp?.mostrar) {
		return null;
	}

	const whatsappUrl = `https://wa.me/${contenido.whatsapp.numero.replace(/[^0-9]/g, '')}?text=${encodeURIComponent(contenido.whatsapp.mensaje)}`;
	...
	return (
		...
		>
			<svg
				...
				viewBox="0 0 24 24"
				fill="currentColor"
				className="h-5 w-5 text-white transition-transform duration-300 group-hover:rotate-12 md:h-7 md:w-7"
			>
			...
			<span
				className={`max-w-0 overflow-hidden whitespace-nowrap text-sm font-semibold text-white transition-all duration-300 ${
					isHovered ? "max-w-[200px] pl-2" : ""
				}`}
			>
				¿Necesitas ayuda?
			</span>
