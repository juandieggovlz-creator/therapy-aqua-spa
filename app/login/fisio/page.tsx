...
export default function FisioLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user.role !== 'fisio') {
        setError('Acceso denegado. Solo fisioterapeutas pueden acceder.');
        setLoading(false);
        return;
      }
        ...
        <div className="bg-white rounded-3xl shadow-2xl p-8 border border-[color:var(--arena-300)]/20">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-gradient-to-r from-[color:var(--oliva-400)] to-green-600 hover:from-green-600 hover:to-[color:var(--oliva-400)] text-white font-bold py-3 px-6 rounded-xl transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none flex items-center justify-center gap-2"
            >
              {loading ? (
                <>
          ...
          <div className="mt-8 p-4 bg-[color:var(--crema-50)] rounded-xl border border-[color:var(--arena-300)]/30">
            ...
            <p className="text-xs text-stone-600">Usuario: <span className="font-mono font-semibold">fisio</span></p>
            <p className="text-xs text-stone-600">Contraseña: <span className="font-mono font-semibold">fisio123</span></p>
          </div>
        </div>
      </div>
    </main>
